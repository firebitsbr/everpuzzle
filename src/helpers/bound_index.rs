...
pub trait BoundIndex {
    /// returns the raw index without bound checks
    fn raw(&self) -> usize;

    /// returns true if the index is in the grid boundaries
    fn in_bounds(&self) -> bool;

    /// returns an optional index dependant on if the index was in the boundaries
