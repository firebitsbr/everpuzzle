...
impl Grid {
	...
	// TODO(Skytrias): alien syntax, get any gridable trait state so we dont have to double down
	fn neighbor_gridable(&self, index: usize, is_2d: bool) -> Vec<Option<Box<&dyn Gridable>>> {
		if is_2d {
				// above, below
				vec![
				 self.gridable_state(index + GRID_WIDTH),
				 
				 if index as i32 - GRID_WIDTH as i32 > 0 {
					 self.gridable_state(index - GRID_WIDTH)
				 } else {
					 None
				 }
	...
	pub fn update(&mut self, app: &mut App) {
		...
		for (_, _, i) in iter_yx_rev() {
				...
				let clear_found = g.children.iter()
				.any(|&i| {
						 let mut found = false;
						 
						 for opt_state in self.neighbor_gridable(i, g.is_2d) {
		...
		// garbage into block clearing
		for (_, _, i) in iter_xy() {
				if let Some(g) = self.garbage_mut(i) {
				if g.state.clear_finished() {
					// TODO(Skytrias): 2d / 1d modes
			...
			if self.block_state(i).filter(|s| s.is_idle()).is_some() {
				if let Some(ib) = (x, y + 1).to_index() {
					...
					let below_garbage_state: GarbageStates = {
						let mut result = GarbageStates::Idle;
						...
						if let Some(index) = parent_index {
							result = self.garbage_state(index).unwrap_or(&GarbageStates::Idle).clone();
						} else {
							result = self.garbage_state(ib).unwrap_or(&GarbageStates::Idle).clone();
						}
	...
	pub fn gridable_state<I: BoundIndex>(&self, index: I) -> Option<Box<&dyn Gridable>> {
		match &self[index] {
				Components::GarbageParent(g) => Some(g.state.as_gridable()),
				Components::Normal(b) => Some(b.state.as_gridable()),
				_ => None,
		}
	}
}
