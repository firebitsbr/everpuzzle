...
use glutin::event::{Event, VirtualKeyCode, ElementState, KeyboardInput, WindowEvent};
...
fn init_ubo(size: usize, index: u32) -> u32 {
	unsafe {
		...
		gl::BufferData(gl::UNIFORM_BUFFER, size as isize, ptr::null(), gl::STATIC_DRAW);
...
pub fn run(width: f32, height: f32, title: &'static str) {
	...
	let window = {
		unsafe {
			glutin::ContextBuilder::new()
				...
				.build_windowed(wb, &event_loop).expect("building window failed")
				.make_current()
				.unwrap()
		}
	};
	...
	unsafe {
		...
		gl::BufferData(gl::UNIFORM_BUFFER, mem::size_of::<Matrix>() as isize, projection.as_ptr() as *const c_void, gl::STATIC_DRAW);
